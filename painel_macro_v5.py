...
Novidades da v5
---------------
- FRETE China→Brasil agora vem de fonte gratuita e AO VIVO: o frete declarado
  por tonelada nas importações da China (Comex Stat / MDIC, US$/t). É a única
  referência da rota China→Brasil com API aberta e gratuita. Substitui o antigo
  FBX (que não tinha API pública gratuita e era valor manual).
- Proxy global OPCIONAL via FRED (Deep Sea Freight PPI), ligado por chave
  gratuita na barra lateral — termômetro direcional do frete marítimo.
- Também disponível: intensidade de frete (frete ÷ FOB) como indicador
  complementar.
- Coleta de dados robusta: sessão HTTP com headers, consulta MDIC unificada
  (FOB + frete + peso), parsing tolerante e alinhamento rótulo × série correto.
- Visual profissional: tema claro por padrão + modo noturno; cartões de KPI,
  cabeçalho e gráficos com identidade consistente.

Instalação:
    pip install streamlit pandas plotly requests

Execução:
    streamlit run painel_macro_v5.py
"""
...
_HEADERS = {
    "User-Agent": "PainelMacro/5.0 (Streamlit dashboard)",
...
def fetch_mdic_general(
    ...
    O DataFrame tem 'data' e uma coluna por métrica. Valores monetários em
    USD milhões; metricKG fica em quilos (unidade original). País = China (160).
    """
    end = datetime.now()
...
FALLBACK = {
    ...
    # Frete CN→BR — frete declarado por TONELADA nas importações da China
    # (MDIC, US$/t). Referência gratuita e específica da rota. Puxa via API.
    "Frete CN→BR": [165, 158, 152, 149, 145, 142, 138, 141,
...
def load_data(n: int = 12) -> dict:
    ...
    # Frete CN→BR (US$/tonelada) = frete / peso líquido.
    ...
    if _ok("metricFreight") and _ok("metricKG"):
        ...
        data["Frete CN→BR"] = per_ton.tolist()[-n:]
        data["Frete CN→BR fonte"] = "MDIC API (metricFreight / metricKG)"
    else:
        data["Frete CN→BR"] = FALLBACK["Frete CN→BR"]
...
INDICATORS: list[Indicator] = [
    ...
    Indicator("Frete CN→BR", 0.25, 100, 250, "US$/t", live=True,
              help="Frete declarado por tonelada nas importações da China (MDIC) — "
                   "referência gratuita e específica da rota China→Brasil."),
...
def main() -> None:
    ...
    with k4:
        v = data["Frete CN→BR"]
        st.metric("Frete China→Brasil", f"US$ {v[-1]:,.0f}/t",
                  f"{(v[-1]/v[0]-1)*100:+.1f}%", delta_color="inverse",
                  help="Frete declarado por tonelada nas importações da China (MDIC).")
    ...
    with c4:
        ...
        st.plotly_chart(make_line_chart(labels, v,
                                        "Frete China→Brasil — US$ por tonelada (importações)",
                                        "#0E7490", t, y_fmt=" US$/t"),
                        use_container_width=True, config={"displayModeBar": False})
        render_source_tag(data.get("Frete CN→BR fonte", "n/d"), t)
    ...
    st.markdown(
        ...
        'O frete US$/t é a melhor referência gratuita da rota China→Brasil '
        '(mensal, defasagem ~30-45 dias).</p>',
        unsafe_allow_html=True,
    )
    ...
    st.markdown(
        ...
        'O frete China→Brasil usa o frete declarado por tonelada nas importações '
        '(MDIC, gratuito). CNY/BRL é entrada manual (sem API gratuita). '
        '<b>Faixas de normalização:</b> '
        'USD/BRL 4,80–6,40 · CNY/BRL 0,65–0,90 · Selic 8–15% · IPCA 12m 2,5–6,5% · '
        'IPCA mês 0–1% · Frete CN→BR $100–$250/t · Imp. China $3.000–$7.000M · '
