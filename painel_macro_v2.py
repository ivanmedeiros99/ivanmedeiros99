...
Execução:
    streamlit run painel_macro_v2.py
...
# Selic, IPCA e USD/BRL são buscados via API; CNY/BRL e FBX são sempre manuais.
FALLBACK = {
...
def load_data(n=12):
    ...
    # Selic meta em vigor — BCB série 432 (taxa deliberada pelo COPOM no período)
    selic_df = fetch_bcb_series(432, n)
    if selic_df is not None and len(selic_df) >= min(10, n):
        data["Selic"] = selic_df["valor"].tolist()[-n:]
    else:
...
def main():
    ...
    st.caption("Série histórica mensal · Atualizado automaticamente via APIs do BCB")

    # ── Sidebar: período + pesos (roda antes de compute_scores) ────────
    ...
    # Gerar rótulos e carregar dados para o período selecionado
    ...
    with col_gauge:
        ...
        weight_str = " · ".join(f"{ind['name']} {int(ind['weight']*100)}%" for ind in INDICATORS)
    ...
    with col3:
        v = data["Selic"]
        delta = v[-1] - v[0]
        st.metric("Taxa Selic em vigor", f"{v[-1]:.2f}% a.a.", f"{delta:+.2f}pp", delta_color="inverse")
        st.plotly_chart(
