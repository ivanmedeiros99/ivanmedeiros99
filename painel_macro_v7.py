...
Novidades da v7
---------------
- SEM nenhum input manual de dado e SEM chaves de API. Todas as séries vêm de
  APIs públicas, gratuitas e sem autenticação:
    • BCB SGS  → Selic, IPCA, USD/BRL
    • BCB PTAX (Olinda) → CNY/BRL (antes era manual; agora ao vivo)
    • Comex Stat / MDIC → importações da China + frete (US$/FEU estimado)
    • FRED (endpoint CSV fredgraph, sem chave) → proxy global de frete marítimo
- O frete China→Brasil continua em US$ por contêiner (FEU), estimado do frete
  declarado no MDIC. O peso médio por FEU segue ajustável (slider de config —
  não é dado, é parâmetro).
- O "índice de frete" no espírito do FBX03/CME é representado pelo proxy global
  do FRED (Deep Sea Freight PPI). Índices de rota cotados por FEU (FBX, SCFI,
  Drewry, Xeneta) não têm API gratuita/sem chave e por isso não são usados.

Instalação:
    pip install streamlit pandas plotly requests

Execução:
    streamlit run painel_macro_v7.py
"""
...
# Peso médio por contêiner FEU (padrão). Ajustável na barra lateral.
DEFAULT_FEU_PAYLOAD_T = 24
...
# ════════════════════════════════════════════════════════════════════
#  Camada de coleta de dados — somente APIs públicas, grátis e sem chave
# ════════════════════════════════════════════════════════════════════

_HEADERS = {
    "User-Agent": "PainelMacro/7.0 (Streamlit dashboard)",
...
@st.cache_data(ttl=3600, show_spinner=False)
def fetch_ptax_currency(symbol: str, n: int = 12) -> Optional[pd.Series]:
    """Cotação PTAX (BCB/Olinda) de uma moeda em BRL, agregada mensal (último dia).

    Gratuito e sem chave. Para CNY, a cotação de venda já é BRL por 1 yuan.
    """
    end = datetime.now()
    start = end.replace(day=1) - pd.DateOffset(months=n + 2)
    ...
    url = (f"{base}?@moeda='{symbol}'"
           f"&@dataInicial='{start:%m-%d-%Y}'"
           f"&@dataFinalCotacao='{end:%m-%d-%Y}'"
    ...
    try:
        ...
        df["valor"] = pd.to_numeric(df["cotacaoVenda"], errors="coerce")
        monthly = (df.dropna(subset=["valor"]).set_index("data")["valor"]
                   .resample("MS").last().ffill())
        return monthly if len(monthly) else None
    except Exception as e:  # noqa: BLE001
        st.warning(f"PTAX {symbol}: {e}")
        return None
...
def _coerce_year_month(item: dict):
    """Extrai (ano, mês) tolerando vários nomes de chave/formatos."""
...
def _coerce_metric(item: dict, *keys) -> Optional[float]:
    """Lê uma métrica numérica testando uma lista de chaves candidatas."""
    for key in keys:
        val = item.get(key)
        if val not in (None, ""):
            try:
...
def fetch_mdic_general(
    ...
    """Consulta unificada ao endpoint /general do Comex Stat (importação).

    Retorna {"df": DataFrame|None, "debug": [tentativas]}.
    Valores monetários em USD milhões; metricKG fica em quilos. País = China (160).
    """
    end = datetime.now()
    ...
    metric_keys = {
        ...
        "metricKG": ("metricKG", "kgLiquido", "kg_liquido", "vlPesoLiquido", "kg"),
    }
    raw_keys = {"metricKG"}  # mantém em kg (sem dividir por 1e6)
...
# ── FRED via CSV (proxy global, gratuito e SEM CHAVE) ────────────────

# Deep Sea Freight Transportation Services (PPI) — proxy direcional do custo
# de frete marítimo de longo curso. Mensal. O endpoint fredgraph.csv não exige
# chave de API.
FRED_FREIGHT_SERIES = "PCU4831114831115"


# O fredgraph.csv pode responder devagar; usamos read timeout maior e retries.
FRED_TIMEOUT = (10, 60)  # (conexão, leitura) em segundos
FRED_RETRIES = 3


@st.cache_data(ttl=3600, show_spinner=False)
def fetch_fred_csv(series_id: str, n: int = 12) -> Optional[pd.DataFrame]:
    """Série do FRED via download CSV público (sem chave). df com 'data' e 'valor'."""
    url = f"https://fred.stlouisfed.org/graph/fredgraph.csv?id={series_id}"
    last_err = None
    for attempt in range(FRED_RETRIES):
        try:
            r = _session().get(url, timeout=FRED_TIMEOUT)
            r.raise_for_status()
            ...
            df["valor"] = pd.to_numeric(df["valor"], errors="coerce")
            df = df.dropna()
            return df.sort_values("data").reset_index(drop=True) if len(df) else None
        except Exception as e:  # noqa: BLE001
            last_err = e
            time.sleep(2 * (attempt + 1))  # backoff: 2s, 4s
    st.warning(f"FRED CSV {series_id}: {last_err}")
    return None


# ════════════════════════════════════════════════════════════════════
#  Fallback (usado apenas se uma API estiver indisponível no momento)
#  Cobre 18 meses encerrando no mês corrente.
# ════════════════════════════════════════════════════════════════════
...
FALLBACK = {
    ...
    # Frete CN→BR /t — frete declarado por TONELADA nas importações da China
    # (MDIC, US$/t). Base para estimar o US$/contêiner (× peso médio por FEU).
    "Frete CN→BR /t": [165, 158, 152, 149, 145, 142, 138, 141,
                       136, 130, 128, 127, 131, 138, 135, 130, 126, 124],
...
def load_data(n: int = 12) -> dict:
    """Carrega todas as séries de APIs públicas (sem chave), com fallback."""
    data: dict = {}
    ...
    # CNY/BRL — BCB PTAX (Olinda), ao vivo e sem chave
    cny = fetch_ptax_currency("CNY", n)
    if cny is not None and len(cny) >= min(10, n):
        data["CNY/BRL"] = cny.tolist()[-n:]
        data["CNY/BRL fonte"] = "BCB PTAX (Olinda)"
    else:
        data["CNY/BRL"] = FALLBACK["CNY/BRL"]
        data["CNY/BRL fonte"] = "fallback (PTAX indisponível)"

    # ── MDIC: importações da China — FOB + FRETE + PESO (uma só consulta) ──
    china = fetch_mdic_general(n + 1, metrics=("metricFOB", "metricFreight", "metricKG"))
    cdf = china.get("df")
    data["Imp. China debug"] = china.get("debug", [])

    def _ok(col):
        return cdf is not None and col in cdf and cdf[col].notna().sum() >= min(6, n)
    ...
    if _ok("metricFOB"):
        ...
        data["Imp. China fonte"] = "fallback (MDIC indisponível)"

    # Frete por tonelada (base para o US$/contêiner calculado em main()).
    # frete em USD MM, peso em kg → (USD·1e6)/kg = USD/kg → ·1000 = USD/t.
    if _ok("metricFreight") and _ok("metricKG"):
        per_ton = (cdf["metricFreight"] * 1_000_000 / cdf["metricKG"] * 1000).round(1)
        data["Frete CN→BR /t"] = per_ton.tolist()[-n:]
        data["Frete CN→BR fonte"] = "MDIC API (metricFreight / metricKG)"
    else:
        data["Frete CN→BR /t"] = FALLBACK["Frete CN→BR /t"]
        data["Frete CN→BR fonte"] = "fallback (MDIC indisponível)"

    # Intensidade de frete (%) = frete / FOB · 100 (indicador complementar)
    if _ok("metricFreight") and _ok("metricFOB"):
        ...
        data["Frete imp. fonte"] = "fallback (MDIC indisponível)"
    ...
    if odf is not None and "metricFOB" in odf and odf["metricFOB"].notna().sum() >= min(6, n):
        ...
        data["Imp. Mob/Orto fonte"] = "fallback (MDIC indisponível)"
...
INDICATORS: list[Indicator] = [
    ...
    Indicator("CNY/BRL", 0.10, 0.65, 0.90, "R$", live=True,
              help="Yuan por real — PTAX (BCB/Olinda), ao vivo e sem chave."),
    ...
    Indicator("Frete CN→BR", 0.25, 1500, 6000, "US$/FEU", live=True,
              help="US$ por contêiner (FEU) estimado: frete declarado nas importações "
                   "da China (MDIC) ÷ contêineres estimados pelo peso médio por FEU."),
...
def render_source_tag(label: str, t: Theme) -> None:
    is_live = "fallback" not in label.lower() and "manual" not in label.lower()
    color = t.good if is_live else t.text_muted
    icon = "● ao vivo" if is_live else "○ fallback"
    st.markdown(
...
def main() -> None:
    # ── Sidebar (somente configurações; nenhum input de dado) ────────
    with st.sidebar:
        ...
        st.markdown("---")
        st.markdown("**Frete por contêiner**")
        payload_feu = st.slider(
            "Peso médio por contêiner (t/FEU)", 8, 28, DEFAULT_FEU_PAYLOAD_T, 1,
            help="Converte o frete declarado do MDIC em US$ por contêiner "
                 "(frete ÷ contêineres estimados pelo peso). Parâmetro de cálculo, "
                 "não um dado de entrada.",
        )
        ...
        for ind in INDICATORS:
            new_weights[ind.name] = st.slider(
                ind.name, 0, 50, int(ind.weight * 100), 5, format="%d%%", help=ind.help,
            )
        ...
        st.markdown("---")
        st.caption("100% de dados via APIs públicas, grátis e sem chave: "
                   "BCB SGS/PTAX · Comex Stat (MDIC) · FRED (CSV).")
    ...
    with st.spinner("Buscando dados atualizados (BCB · MDIC · FRED)…"):
        data = load_data(n_months)

    # US$ por contêiner (FEU) = US$/t × peso médio por FEU (ajustável na sidebar)
    per_ton = data.get("Frete CN→BR /t", FALLBACK["Frete CN→BR /t"])
    data["Frete CN→BR"] = [round(x * payload_feu) for x in per_ton]
    ...
    st.markdown(
        ...
        <div class="pm-header">
          <div>
            ...
            <div class="pm-sub">Brasil × China · BCB + Comex Stat (MDIC) + FRED · série mensal</div>
          </div>
    ...
    with k4:
        v = data["Frete CN→BR"]
        st.metric("Frete China→Brasil", f"US$ {v[-1]:,.0f}/FEU",
                  f"{(v[-1]/v[0]-1)*100:+.1f}%", delta_color="inverse",
                  help="US$ por contêiner (FEU) estimado do frete declarado das "
                       "importações da China (MDIC).")
    ...
    with c2:
        ...
        st.plotly_chart(make_line_chart(labels, v, "CNY / BRL — yuan por real",
                        ...
                        config={"displayModeBar": False})
        render_source_tag(data.get("CNY/BRL fonte", "n/d"), t)
    ...
    with c4:
        v = data["Frete CN→BR"]
        st.plotly_chart(make_line_chart(labels, v,
                                        "Frete China→Brasil — US$ por contêiner (FEU, estimado)",
                                        "#0E7490", t, y_fmt=" US$/FEU"),
                        use_container_width=True, config={"displayModeBar": False})
        render_source_tag(f"{data.get('Frete CN→BR fonte', 'n/d')} · {payload_feu} t/FEU", t)
    ...
    # ── Seção 3: Frete & comércio China (MDIC + proxy FRED) ──────────
    st.markdown('<hr class="pm-divider">', unsafe_allow_html=True)
    st.markdown('<div class="pm-section">Frete & comércio com a China</div>',
                unsafe_allow_html=True)
    st.markdown(
        '<p class="pm-caption">Rota Brasil↔China via Comex Stat (MDIC). '
        f'US$/contêiner estimado (frete ÷ contêineres, {payload_feu} t/FEU). '
        'O proxy global de frete vem do FRED (Deep Sea Freight PPI) — '
        'substituto livre e sem chave para índices de rota pagos como FBX/SCFI.</p>',
        unsafe_allow_html=True,
    )
    ...
    # Proxy global de frete via FRED (CSV, sem chave) — sempre ativo
    fred_df = fetch_fred_csv(FRED_FREIGHT_SERIES, n_months)
    if fred_df is not None and len(fred_df) >= 2:
        fred_vals = fred_df["valor"].tolist()[-n_months:]
        fred_lbls = [f"{MONTH_NAMES[d.month-1]}/{str(d.year)[2:]}"
                     for d in fred_df["data"]][-n_months:]
        st.plotly_chart(make_line_chart(
            fred_lbls, fred_vals,
            "Proxy global de frete marítimo — Deep Sea Freight PPI (FRED, índice)",
            "#0891B2", t), use_container_width=True, config={"displayModeBar": False})
        render_source_tag("FRED CSV (PCU4831114831115)", t)
    else:
        st.info("Proxy FRED indisponível no momento (a fonte é gratuita e sem chave).")
    ...
    with c10:
        ...
        for key, lbl in [("Imp. China debug", "China (FOB + frete + peso)"),
    ...
    st.markdown('<hr class="pm-divider">', unsafe_allow_html=True)
    st.markdown(
        '<p class="pm-caption"><b>Fontes (todas públicas, grátis e sem chave):</b> '
        'Banco Central do Brasil (SGS e PTAX/Olinda) · MDIC Comex Stat · '
        'FRED (download CSV). Frete China→Brasil estimado em US$/contêiner (FEU) '
        'a partir do frete declarado nas importações. '
        '<b>Faixas de normalização:</b> '
        'USD/BRL 4,80–6,40 · CNY/BRL 0,65–0,90 · Selic 8–15% · IPCA 12m 2,5–6,5% · '
        'IPCA mês 0–1% · Frete CN→BR $1.500–$6.000/FEU · Imp. China $3.000–$7.000M · '
