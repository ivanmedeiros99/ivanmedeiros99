...
Novidades da v6
---------------
- FRETE expresso em US$ POR CONTÊINER (FEU), e não mais por tonelada. Como o
  Comex Stat/MDIC não informa quantidade de contêineres, o valor é estimado:
  frete declarado ÷ (peso total ÷ peso médio por FEU). O peso médio por FEU é
  ajustável na barra lateral (padrão 24 t), o que aproxima o número das tarifas
  reais de mercado. Fonte gratuita e automática (mensal, defasagem ~30-45 dias).
- Proxy global OPCIONAL via FRED (Deep Sea Freight PPI), ligado por chave grátis.
- Intensidade de frete (frete ÷ FOB) mantida como indicador complementar.
- Coleta robusta (sessão HTTP, consulta MDIC unificada FOB+frete+peso) e visual
  profissional com tema claro + modo noturno.

Instalação:
    pip install streamlit pandas plotly requests

Execução:
    streamlit run painel_macro_v6.py
"""
...
_HEADERS = {
    "User-Agent": "PainelMacro/6.0 (Streamlit dashboard)",
...
def fetch_mdic_general(
    ...
    Valores monetários em USD milhões; metricKG fica em quilos (unidade
    original). País = China (160).
    """
    end = datetime.now()
...
# ── FRED (proxy global gratuito, opcional) ───────────────────────────

# Deep Sea Freight Transportation Services (PPI) — proxy direcional do custo
# de frete marítimo de longo curso. Mensal, gratuito via API do FRED
# (chave grátis em https://fredaccount.stlouisfed.org/apikeys).
FRED_FREIGHT_SERIES = "PCU4831114831115"


@st.cache_data(ttl=3600, show_spinner=False)
def fetch_fred_series(series_id: str, api_key: str, n: int = 12) -> Optional[pd.DataFrame]:
    """Observações de uma série do FRED (JSON). Retorna df com 'data' e 'valor'."""
    if not api_key:
        return None
    ...
    try:
        ...
        rows = [{"data": pd.to_datetime(o["date"]), "valor": float(o["value"])}
...
#  dados do MDIC (incl. frete) vêm de API; CNY/BRL é manual.
# ════════════════════════════════════════════════════════════════════
...
FALLBACK = {
    ...
    # Intensidade de frete = frete declarado / FOB (%), importações China
...
def load_data(n: int = 12) -> dict:
    ...
    O frete é entregue como 'Frete CN→BR /t' (US$/tonelada). A conversão
    para US$/contêiner (FEU) é feita em main(), pois depende do peso médio
    por FEU escolhido na barra lateral.
    """
    data: dict = {}
    ...
    # CNY/BRL — manual (sem API pública gratuita confiável)
    ...
    # Frete por tonelada (base para estimar o US$/contêiner em main()).
    ...
    if _ok("metricFreight") and _ok("metricKG"):
        ...
        data["Frete CN→BR fonte"] = "fallback manual"
...
def render_source_tag(label: str, t: Theme) -> None:
    live = "API" in label or "MDIC" in label or "FRED" in label
...
def main() -> None:
    ...
    with st.sidebar:
        ...
        payload_feu = st.slider(
            ...
            help="Converte o frete declarado do MDIC em US$ por contêiner "
                 "(frete ÷ contêineres estimados pelo peso). Ajuste ao perfil "
                 "de carga das suas importações.",
        )

        st.markdown("---")
        st.markdown("**Entrada manual**")
        cny_latest = st.number_input(
            "CNY/BRL, mês atual (sem API gratuita)",
            min_value=0.50, max_value=1.20,
            value=float(FALLBACK["CNY/BRL"][-1]), step=0.01, format="%.2f",
        )

        st.markdown("---")
        st.markdown("**Proxy de frete global (FRED)**")
        st.caption("Opcional. Chave gratuita em fredaccount.stlouisfed.org/apikeys.")
        fred_key = st.text_input("FRED API key", value="", type="password",
                                 placeholder="cole a chave para ligar o proxy")
    ...
    data["CNY/BRL"][-1] = float(cny_latest)  # entrada manual no mês corrente
    ...
    st.markdown('<div class="pm-section">Frete & comércio com a China (Comex Stat / MDIC)</div>',
                unsafe_allow_html=True)
    st.markdown(
        '<p class="pm-caption">Fonte: api-comexstat.mdic.gov.br · '
        'frete e peso declarados nas importações da China. '
        f'O US$/contêiner é estimado (frete ÷ contêineres, {payload_feu} t por FEU); '
        'fonte gratuita, mensal, defasagem ~30-45 dias.</p>',
        unsafe_allow_html=True,
    )
    ...
    # Proxy global opcional via FRED
    fred_df = fetch_fred_series(FRED_FREIGHT_SERIES, fred_key, n_months) if fred_key else None
    if fred_df is not None and len(fred_df) >= 2:
        ...
        render_source_tag("FRED API (PCU4831114831115)", t)
    elif fred_key:
        st.info("Proxy FRED: não foi possível carregar a série (verifique a chave).")
    ...
    st.markdown(
        ...
        'MDIC Comex Stat · FRED (proxy opcional) · IBGE. '
        'O frete China→Brasil é estimado em US$/contêiner (FEU) a partir do frete '
        'declarado nas importações (MDIC, gratuito). CNY/BRL é entrada manual. '
